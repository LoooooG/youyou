...
public class HnPrivateLetterDetailBiz {
    ...
    public HnPrivateLetterDetailBiz(BaseActivity context) {
    ...
    public void requestToPrivateLetterDetail(String chatRoomId, String dialogId) {
        ...
        HnHttpUtils.postRequest(HnLiveUrl.PRIVATELETTER_DETAIL, param, TAG, new HnResponseHandler<PrivateLetterDetailModel>(context, PrivateLetterDetailModel.class) {
            @Override
    ...
    public void requestToFollow(String mUid) {
        ...
        RequestParams param = new RequestParams();
        param.put("user_id", mUid);
        HnUserControl.addFollow(mUid, "", new HnUserControl.OnUserOperationListener() {
            @Override
     ...
     * @param mUid    对方id
     */
    public void requestToSendPrivateLetter(String content, String mUid, String chatRoomId) {
        if (TextUtils.isEmpty(content)) {
            HnToastUtils.showToastShort(context.getString(R.string.live_letter_not_empty));
            return;
        }
        RequestParams param = new RequestParams();
        param.put("chat_room_id", chatRoomId);
        ...
        HnHttpUtils.postRequest(HnLiveUrl.SEND_PRIVATELETTER, param, TAG, new HnResponseHandler<HnSendPriMsgModel>(context, HnSendPriMsgModel.class) {
            ...
            public void hnSuccess(String response) {
                if (model.getC() == 0) {
                    if (listener != null) {
                        listener.requestSuccess("send_msg", response, model);
                    }
                } else {
                    if (listener != null) {
    ...
    public void showGiftView(boolean showAnimation, LayoutTransition transitioner, RelativeLayout mGiftContainer, RelativeLayout mBottomCon, LinearLayout mContainer, EditText mPrivateChatInput, LinearLayout mOutcontainer) {
        ...
        HnLiveUtils.hideSoftKeyBoard(mPrivateChatInput, context);

        mGiftContainer.getLayoutParams().height = HnLiveScreentUtils.dp2px(context, 278);
        mBottomCon.setVisibility(View.VISIBLE);
        mGiftContainer.setVisibility(View.VISIBLE);
        mContainer.setVisibility(View.GONE);
        context.getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_ALWAYS_HIDDEN);

        int lockHeight = HnLiveScreentUtils.getAppContentHeight(context);
    ...
    public void showEmotionView(boolean showAnimation, LayoutTransition transitioner, LinearLayout mContainer, RelativeLayout mBottomCon, RelativeLayout mGiftContainer, LinearLayout mOutcontainer, EditText mPrivateChatInput) {
        ...
        HnLiveUtils.hideSoftKeyBoard(mPrivateChatInput, context);
        mContainer.getLayoutParams().height = HnLiveScreentUtils.dp2px(context, 278);
        mBottomCon.setVisibility(View.VISIBLE);
     ...
     *
     * @param chatRoomId 对方id
     */
    public void requestToExitMsgDetail(String chatRoomId) {
        if (TextUtils.isEmpty(chatRoomId)) return;
        RequestParams param = new RequestParams();
        param.put("chat_room_id", chatRoomId);
        HnHttpUtils.postRequest(HnLiveUrl.USER_CHAT_IGNORE_UNREAD, param, TAG, new HnResponseHandler<BaseResponseModel>(context, BaseResponseModel.class) {
            @Override
            public void hnSuccess(String response) {
